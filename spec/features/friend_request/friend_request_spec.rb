...
RSpec.feature " Following friend " do
  before do 
    ...
    @isack = User.create(name:"Isack",email:"isack@gmail.com",password:"123456")
    @makala = User.create(name:"Makala",email:"makala@gmail.com",password:"123456")
    login_as(@juma)
  end
  scenario 'List of all users with friendship status' do 
    ...
    find(link, :visible => false)
    href = "friendships?friend_id=#{@maya.id}"
    # expect(page).to have_link("Remove Request", :href => href)
  end
end